...
def test_field_coverage_counts_presence_and_types_without_values():
    ...
    assert summary == {
        "events": 3,
        "fields": {
            "request_id": {"present": 2, "missing": 1, "coverage": 2 / 3, "populated": 2, "populated_rate": 2 / 3, "nulls": 0, "null_rate": 0.0, "types": {"string": 2}, "type_drift": False},
            "status": {"present": 1, "missing": 2, "coverage": 1 / 3, "populated": 1, "populated_rate": 1 / 3, "nulls": 0, "null_rate": 0.0, "types": {"integer": 1}, "type_drift": False},
        },
    }
...
def test_field_coverage_is_deterministic_and_accepts_generators():
    ...
    assert summary["fields"]["z"] == {"present": 2, "missing": 0, "coverage": 1.0, "populated": 2, "populated_rate": 1.0, "nulls": 0, "null_rate": 0.0, "types": {"integer": 2}, "type_drift": False}
...
def test_field_coverage_deduplicates_keys_with_same_display_identity():
    ...
    summary = summarize_field_coverage(events)
    assert summary["fields"]["1"] == {"present": 2, "missing": 0, "coverage": 1.0, "populated": 2, "populated_rate": 1.0, "nulls": 0, "null_rate": 0.0, "types": {"string": 3}, "type_drift": False}
...
def test_field_nullability_prefers_populated_normalized_alias_per_event():
    ...
    assert field["populated"] == 1
    assert field["populated_rate"] == 0.5
...
def test_field_coverage_normalizes_unicode_and_control_characters():
    ...
    summary = summarize_field_coverage(events)
    assert summary["fields"]["requestid"] == {"present": 2, "missing": 0, "coverage": 1.0, "populated": 2, "populated_rate": 1.0, "nulls": 0, "null_rate": 0.0, "types": {"string": 3}, "type_drift": False}
    assert summary["fields"]["status code"] == {"present": 1, "missing": 1, "coverage": 0.5, "populated": 1, "populated_rate": 0.5, "nulls": 0, "null_rate": 0.0, "types": {"integer": 1}, "type_drift": False}
...
def test_field_coverage_uses_explicit_identity_for_empty_keys():
    ...
    assert summary["fields"] == {"<empty>": {"present": 1, "missing": 0, "coverage": 1.0, "populated": 1, "populated_rate": 1.0, "nulls": 0, "null_rate": 0.0, "types": {"integer": 1}, "type_drift": False}}
...
def test_field_coverage_classifies_coarse_types_and_nullability_without_values():
    ...
    assert summary["fields"]["nullable"]["populated"] == 1
    assert summary["fields"]["nullable"]["populated_rate"] == 1 / 3
    ...
    assert summary["fields"]["drifting"]["populated"] == 2
    assert summary["fields"]["drifting"]["populated_rate"] == 2 / 3
