...
def _open_input(path: Path):
    """Open local text logs, transparently decompressing .gz files."""
    if str(path) == "-":
...
def _analyze(path: Path, format: str, output_format: str, levels: set[str] | None,
             contains: str | None, sources: set[str] | None, error_threshold: int,
             repeat_threshold: int, burst_threshold: int, burst_window_seconds: int,
             window_minutes: int | None, fail_on_severity: str | None) -> int:
    events = []
    total_input = 0
    parse_errors = 0
    ...
    findings = [finding.to_dict() for finding in detect_anomalies(
        matched, error_threshold=error_threshold, repeat_threshold=repeat_threshold,
        burst_threshold=burst_threshold, burst_window_seconds=burst_window_seconds,
    )]
    ...
    report.update({
        ...
        "detection_config": {"error_threshold": error_threshold, "repeat_threshold": repeat_threshold,
                             "burst_threshold": burst_threshold, "burst_window_seconds": burst_window_seconds},
        "findings": findings,
    })
    if window_minutes is not None:
        report["time_baseline"] = {"window_minutes": window_minutes,
    ...
    if fail_on_severity is not None:
        ...
        if any(_SEVERITY_RANK.get(finding["severity"], 0) >= threshold for finding in findings): return 3
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "analyze":
        try:
            return _analyze(args.path, args.format, args.output_format,
                set(args.levels) if args.levels else None, args.contains,
                set(args.sources) if args.sources else None, args.error_threshold,
                args.repeat_threshold, args.burst_threshold, args.burst_window_seconds,
